...
Preamble:
The [DADA2 pipeline](https://benjjneb.github.io/dada2/tutorial.html) is used as a method to correct errors that are introduced into sequencing data during amplicon sequencing. The output of this pipeline is a table of amplicon sequence variants (ASVs), as opposed to the traditional OTUs seen in other amplicon sequencing workflows that cluster similar sequences into a single OTU. DADA2 is capable of resolving biological differences of 1 or 2 nucleotides, producing ASVs from amplicon data that are of higher resolution than OTUs.

### Requirements & Installations
DADA2 is an open-source [R package](https://github.com/benjjneb/dada2) that will allow you to run through the entire pipeline, including steps to filter, dereplicate, identify chimeras, and merge paired-end reads.

To use DADA2, you must download and install R for your operating system. This can be done [here](https://cloud.r-project.org/). You should use the latest version of R/

If you would like, download and install RStudio, which is an integrated development environment (IDE) for R programming. This can be done through [this link](https://www.rstudio.com/download).
...
=======
To install the latest version of the DADA2 package, type the following:

```
BiocManager::install("dada2", version = "3.9")
```

*Note that if you previously had R and Bioconductor installed, you may need to update them to the most recent versions.*
>>>>>>> 934decbba6921c53eff869685e83f504230c3cf1
...
You will also need to specify the primers sequences you used as the "FWD" and "REV" variables. These will be removed. To check that the primer sequences are correct and are present, we start off by creating all possible orientations of the forward and reverse sequences.
...
<<<<<<< HEAD
Next, since the DADA2 workflow does not allow any ambiguous bases (Ns), we will remove all sequences containing Ns. Then, we will count the number of times our primers are observed in the remaining sequences. At this point, you should expect to find the forward primer in the forward reads in the forward orientation and reverse reads in the reverse orientation. You should also expect to find the reverse primer in the reverse reads in the forward orientation and forward reads in the reverse orientation. 
...
<<<<<<< HEAD
Now that we have confirmed the presence of the correct primers, we can use cutadapt to remove them. We will also check that they have been successfully removed. You should expect to see 0s across the table that is generated. To use cutadapt, you will need to specify the path of it on your machine (we used "/home/stefan/.local/bin/cutadapt").
...
<<<<<<< HEAD
Now that primers are removed, we can officially begin the DADA2 pipeline. We start with plotting quality profiles of our sequences to determine suitable quality filtering parameters. 
=======
...
``` {r}
# Load the classifier into R, then parse the headers by their taxonomic levels.
train <- readDNAStringSet("~/Desktop/ITS2_nematode_dada2.fasta")
s <- strsplit(names(train), ";")
  superkingdom <- sapply(s, `[`, 1)
  kingdom <- sapply(s, `[`, 2)
  phylum <- sapply(s, `[`, 3)
  class <- sapply(s, `[`, 4)
  order <- sapply(s, `[`, 5)
